...
<?php } ?>
  ...
  <div id="planet-info">
    ...
    echo "<br>";
    echo "<br>";
    echo tr("Temperatura").": ".$planet['temperature']."&deg;C";
    echo "<br>";
    echo "<br>";
...
?></label>
          <input type="number" max="6" min="1" value="1" name="passengers" id="passengers"/><br><br>
        </div>
        <div id="cabin-div">
...
?>
  ...
  <?php
        for ($i = 1;$i < $_POST['passengers'];$i = $i + 1) {
            ...
            echo "<input type=\"text\" pattern =\"([a-z]*[A-Z]*)*\" name=\"name_guest$i\" id=\"name_guest$i\" required/><br><br>";

            echo "<label for=\"lastname_guest$i\">";
            tr("Cognome ospite numero");
            echo " $i <br></label>";
            tr("(Solo lettere maiuscole o minuscole)");
            echo "<input type=\"text\" pattern =\"([a-z]*[A-Z]*)*\" name=\"lastname_guest$i\" id=\"lastname_guest$i\" required/><br>";
            echo "</div>";
        }
?>
  <input type="submit" value="<?php tr("Invia");?>"/>
